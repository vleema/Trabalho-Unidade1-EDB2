...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\If{tamanho da \textbf{lista} $\leq 1$} \Comment Uma lista com um elemento está ordenada
		\State \Return
		\EndIf
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require{$\mathbf{listaResultado} = 0, 0, \ldots, 0$ $\mathbf{listaEsquerda} = x_0, x_1, \ldots, x_{N - 1}$, $\mathbf{listaDireita} = y_0, y_1, \ldots, y_{M - 1}$}
		\Ensure{A \textbf{listaResultado} ordenada com os elementos da \textbf{listaEsquerda} e \textbf{listaDireita}}
		\Statex
		\Function{Merge}{$\mathbf{listaResultado}$,
			$\mathbf{listaEsquerda}$,
			$\mathbf{listaDireita}$}
