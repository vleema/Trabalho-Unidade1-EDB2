...
\begin{figure}[!ht]
	\centering
	\includegraphics[scale=0.34]{latex/figures/quick/q1.png}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\If{$\text{low} < \text{high}$}
		\State $\text{pivo\_index} \gets$ \Call{particao}{$A, \text{low}, \text{high}$}

		\If{$\text{pivo\_index} > 0$}
		\State Adicione $(\text{low}, \text{pivo\_index} - 1)$ na pilha \Comment{Lado esquerdo}
		\EndIf
		\State Adicione $(\text{pivo\_index} + 1, \text{high})$ na pilha \Comment{Lado direito}
		\EndIf
		\EndWhile
		\EndFunction
	\end{algorithmic}
...
\]
    ...
    De forma análoga, podemos dizer que este algoritmo será \( \Omega(nlogn) \) e \( \Theta(nlogn) \).
